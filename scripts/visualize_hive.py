...
try:
    # Read Query 1 data
    q1_path = './results/hive_results/query1/000000_0'
    if not os.path.exists(q1_path):
        print(f"✗ Error: Query 1 results file not found at {q1_path}")
        print("Please ensure Hive queries have been executed first.")
        sys.exit(1)
    ...
    if q1_df.empty:
        print("✗ Error: Query 1 results file is empty")
        sys.exit(1)
except Exception as e:
    print(f"✗ Error reading Query 1 data: {str(e)}")
    sys.exit(1)
...
try:
    ...
    if total_count == 0:
        print("✗ Error: No traffic data found in Query 1 results")
        sys.exit(1)

    print(f"   - Loaded {len(q1_df)} records")
    print(f"   - Total traffic records: {total_count:,}")
except Exception as e:
    print(f"✗ Error processing Query 1 data: {str(e)}")
    sys.exit(1)
...
try:
    if normal_count > 0 or attack_count > 0:
        ...
        print("   ⚠ Warning: Skipping traffic distribution pie chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create traffic distribution pie chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    ...
    if not attack_data.empty and attack_data['count'].sum() > 0:
        ...
        print("   ⚠ Warning: Skipping attack categories bar chart - no attack data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create attack categories bar chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    ...
    if not attack_data.empty:
        ...
        print("   ⚠ Warning: Skipping bytes comparison chart - no attack data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create bytes comparison chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    # Read Query 2 data
    q2_path = './results/hive_results/query2/000000_0'
    if not os.path.exists(q2_path):
        print(f"✗ Error: Query 2 results file not found at {q2_path}")
        print("Please ensure Hive queries have been executed first.")
        sys.exit(1)
    ...
    if q2_df.empty:
        print("✗ Error: Query 2 results file is empty")
        sys.exit(1)
except Exception as e:
    print(f"✗ Error reading Query 2 data: {str(e)}")
    sys.exit(1)
...
try:
    ...
    print(f"✗ Error processing Query 2 data: {str(e)}")
    sys.exit(1)

# Visualization 2.1: Attack Percentage by Protocol-Service (Horizontal Bar)
try:
    if not q2_df.empty and q2_df['attack_percentage'].sum() > 0:
        ...
        print("   ⚠ Warning: Skipping protocol attack percentage chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create protocol attack percentage chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    if not q2_df.empty:
        ...
        print("   ⚠ Warning: Skipping connection vs attack scatter plot - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create connection vs attack scatter plot: {str(e)}")
    if 'plt' in locals():
        plt.close()

# Visualization 2.3: Top 5 Protocols by Connection Volume (Stacked Bar)
try:
    if not q2_df.empty and len(q2_df) >= 1:
        fig, ax = plt.subplots(figsize=(12, 8))
        top5 = q2_df.nlargest(min(5, len(q2_df)), 'connection_count')
        ...
        ax.set_title(f'Top {len(top5)} Protocol/Service by Connection Volume', fontsize=16, weight='bold', pad=20)
        ax.set_xticks(x)
        ...
        print("   ⚠ Warning: Skipping top protocols stacked bar chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create top protocols stacked bar chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    # Read Query 4 data
    q4_path = './results/hive_results/query4/000000_0'
    if not os.path.exists(q4_path):
        print(f"✗ Error: Query 4 results file not found at {q4_path}")
        print("Please ensure Hive queries have been executed first.")
        sys.exit(1)
    ...
    if q4_df.empty:
        print("✗ Error: Query 4 results file is empty")
        sys.exit(1)
except Exception as e:
    print(f"✗ Error reading Query 4 data: {str(e)}")
    sys.exit(1)
...
try:
    ...
    print(f"✗ Error processing Query 4 data: {str(e)}")
    sys.exit(1)

# Visualization 4.1: Attack Count by Connection State
try:
    if not q4_df.empty and q4_df['attack_count'].sum() > 0:
        fig, ax = plt.subplots(figsize=(12, 8))
        q4_sorted = q4_df.sort_values('attack_count', ascending=True)

        colors = plt.cm.Reds(q4_sorted['attack_count'] / q4_sorted['attack_count'].max()) if q4_sorted['attack_count'].max() > 0 else ['red']
        ...
        print("   ⚠ Warning: Skipping attacks by state chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create attacks by state chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    if not q4_df.empty:
        ...
        ax4 = axes[1, 1]
        attack_pct = (q4_df['attack_count'] / q4_df['total_connections'] * 100).fillna(0)  # Handle division by zero
        ...
        print("   ⚠ Warning: Skipping state statistics multi-chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create state statistics multi-chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    if not q4_df.empty:
        ...
        for i, (_, row) in enumerate(q4_df.iterrows()):
            ...
            ax.add_patch(plt.Rectangle((i-0.3, box_bottom), 0.6, max(0, box_top - box_bottom),
        ...
        print("   ⚠ Warning: Skipping bytes distribution chart - no data available")
except Exception as e:
    print(f"   ⚠ Warning: Failed to create bytes distribution chart: {str(e)}")
    if 'plt' in locals():
        plt.close()
...
try:
    ...
    with open(summary_file, 'w') as f:
        ...
        try:
            ...
            attack_pct = (attack_count / total_count * 100) if total_count > 0 else 0
            
            attack_records = q1_df[q1_df['traffic_type']=='Attack']
            if not attack_records.empty:
                most_common_attack = attack_records.iloc[0]
            else:
                most_common_attack = {'attack_cat': 'No Data', 'count': 0}
            ...
            f.write(f"Number of Attack Categories: {len(attack_records)}\n")
            f.write(f"Most Common Attack: {most_common_attack['attack_cat']}\n")
            f.write(f"  - Count: {int(most_common_attack['count']):,}\n\n")
        except Exception as e:
            f.write(f"QUERY 1: Error generating summary - {str(e)}\n\n")
        ...
        try:
            if not q2_df.empty:
                ...
                f.write("QUERY 2: No protocol data available\n\n")
        except Exception as e:
            f.write(f"QUERY 2: Error generating summary - {str(e)}\n\n")
        ...
        try:
            if not q4_df.empty:
                ...
                f.write("QUERY 4: No connection state data available\n\n")
        except Exception as e:
            f.write(f"QUERY 4: Error generating summary - {str(e)}\n\n")
    ...
    print(f"   ⚠ Warning: Failed to generate summary report: {str(e)}")
